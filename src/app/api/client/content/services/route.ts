...
export async function GET() {
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.from("hvac_services").select("*").eq("client_id", clientId).order("sort_order");
...
export async function POST(request: Request) {
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.from("hvac_services").insert({ ...filtered, client_id: clientId }).select().single();
  if (error) { console.error(error.message); return NextResponse.json({ error: "Operation failed" }, { status: 500 }); }
  return NextResponse.json(data);
}
