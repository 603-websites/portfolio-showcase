...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.from("hvac_promotions").update(filtered).eq("id", id).eq("client_id", clientId).select().single();
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const admin = createAdminClient();
  const { error } = await admin.from("hvac_promotions").delete().eq("id", id).eq("client_id", clientId);
  if (error) { console.error(error.message); return NextResponse.json({ error: "Operation failed" }, { status: 500 }); }
